...
impl CallDecl {
    ...
    pub fn args_for_entity_handler(
        ...
        self.expr
            ...
            .map(|(arg, expected_type)| match arg {
                CallArg::HexAddress(address) => match expected_type {
                ...
                CallArg::Subgraph(SubgraphArg::EntityParam(name)) => {
                    ...
                    match (&expected_type, value) {
                        ...
                        (ParamType::Bytes, Value::Bytes(b)) => {
                            Ok(Token::Bytes(b.as_ref().to_vec()))
                        }
                        ...
                        (ParamType::Uint(_), Value::Int(i)) if *i >= 0 => {
                            Ok(Token::Uint((*i).into()))
                        }
                        ...
                        (ParamType::Array(inner_type), Value::List(values)) => {
                            let tokens: Result<Vec<Token>, Error> = values
                                .iter()
                                .map(|v| CallDecl::convert_value_to_token(v, inner_type.as_ref()))
                                .collect();
                            Ok(Token::Array(tokens?))
                        }
                        _ => Err(anyhow!(
                            "type mismatch for param '{name}': cannot convert {:?} to {:?}",
                            value,
                            expected_type
                        )),
                    }
    ...
    // Helper function for converting a single value
    fn convert_value_to_token(value: &Value, param_type: &ParamType) -> Result<Token, Error> {
        match (param_type, value) {
            ...
            (ParamType::Uint(_), Value::BigInt(i)) => Ok(Token::Uint(i.to_unsigned_u256())),
            _ => Err(anyhow!(
...
impl<'de> de::Deserialize<'de> for CallDecls {
        ...
        D: de::Deserializer<'de>,
    {
        ...
        // First parse all expressions
        let parsed_decls: Vec<CallDecl> = decls
            .into_iter()
            .map(|(name, expr)| {
        ...
        // Then validate no mixing of Ethereum and Subgraph across all expressions
        let has_ethereum = parsed_decls.iter().any(|decl| {
            matches!(decl.expr.address, CallArg::Ethereum(_))
                || decl
                    .expr
                    .args
                    .iter()
        ...
        let has_subgraph = parsed_decls.iter().any(|decl| {
            matches!(decl.expr.address, CallArg::Subgraph(_))
                || decl
                    .expr
                    .args
                    .iter()
        ...
        if has_ethereum && has_subgraph {
            return Err(de::Error::custom(
                "Cannot mix Ethereum and Subgraph args across call expressions",
            ));
        }

        Ok(CallDecls {
            decls: Arc::new(parsed_decls),
            readonly: (),
        })
    }
}
